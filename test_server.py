...
def test_backend_with_sample_csv():
    # Path to sample CSV
    ...
    with open(csv_path, "rb") as f:
        response = client.post("/detect", files={"file": ("sample_transactions.csv", f, "text/csv")})
    ...
    if response.status_code == 200:
        ...
        if rings:
            ...
            for ring in rings[:3]: # Show first 3 rings
        ...
        if suspicious:
            print(f"\n--- Top Suspicious Accounts ({len(suspicious)}) ---")
            for acc in suspicious[:3]: # Show top 3 suspicious
                print(f"Account: {acc['account_id']}, Score: {acc['suspicion_score']:.2f}, Patterns: {acc['detected_patterns']}")
